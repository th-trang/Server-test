...
function insertInitialValues(){
  ...
  //Query
  pool.query(sql, [initialValues], (error, results, fields) => {
    if (error){
      console.error('Lỗi khi chèn dữ liệu ban đầu: ' + error.message);
      return;
    }
    console.log('Dữ liệu ban đầu đã được chèn thành công vào cơ sở dữ liệu.');
  });
}
...
module.exports = {
  pool: pool.promise(),
  insertInitialValues: insertInitialValues,
